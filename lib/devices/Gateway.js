...
module.exports = class Gateway extends GenericDevice {
        ...
        switch (message.cmd) {
            ...
            case 'read_ack':
                ...
                if (data.rgb || data.rgb === 0) {
                    ...
                    this.decimalColor = colorToNumber(this.red, this.green, this.blue);
                }
                if (message.cmd === 'report')
                    return ({ msg: 'event', data: { sid: this.sid, model: this.model, color: this.decimalColor, intensity: this.intensity, illumination: this.illumination } });
                break;
        }
    }
    ...
    setColor(color, intensity) {
        const _intensity = (intensity || intensity === 0) ? intensity : 10;
