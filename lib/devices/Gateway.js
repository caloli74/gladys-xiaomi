const crypto = require('crypto');
const GenericDevice = require('./GenericDevice.js');
const AQARA_IV = Buffer.from([0x17, 0x99, 0x6d, 0x09, 0x3d, 0x28, 0xdd, 0xb3, 0xba, 0x69, 0x5a, 0x2e, 0x6f, 0x58, 0x56, 0x2e]);

module.exports = class Gateway extends GenericDevice {
    constructor(sid, sendSocket) {
        super(sid, 'gateway');

        this.token = '';
        this.illumination = 0;
        this.intensity = 0;
        this.color = { r: 0, g: 0, b: 0 };
        this.charge = 'N/A';
        this.sendSocket = sendSocket;

        // ask for devices list
        this.sendSocket('{"cmd": "get_id_list"}');
    }

    deviceMessage(message) {
        super.deviceMessage(message);

        // get details for each device
        switch (message.cmd) {
            case 'get_id_list_ack':
                const msg = `{"cmd": "read", "sid": "${this.sid}"}`;
                this.sendSocket(msg);

                var sids = JSON.parse(message.data);
                for (var sid of sids) {
                    const msg = `{"cmd": "read", "sid": "${sid}"}`;
                    this.sendSocket(msg);
                }
                return ({msg:'ready'});
                break;

            case 'heartbeat':
                this.token = message.token;
                break;

            case 'report':
            case 'read_ack':
                const data = JSON.parse(message.data);
                if (data.illumination || data.illumination === 0)
                    this.illumination = data.illumination;

                if (data.rgb || data.rgb === 0) {
                    const rgb = data.rgb;
                    this.intensity = Math.trunc(rgb / Math.pow(2, 24));
                    this.color.r = Math.trunc((rgb % Math.pow(2, 24)) / Math.pow(2, 16));
                    this.color.g = Math.trunc((rgb % Math.pow(2, 16)) / Math.pow(2, 8));
                    this.color.b = rgb % Math.pow(2, 8);
                }
                if (message.cmd === 'report')
                    return({msg:'event', data:{sid: this.sid, model: 'gateway', color: this.color, intensity: this.intensity, illumination: this.illumination}});
                break;
        }
    }

    playSound(sound, volume) {
        const msg = `{"cmd": "write", "model": "gateway", "sid": "${this._sid}", "data": "{\\"mid\\":${sound}, \\"vol\\":${volume}, \\"key\\": \\"${this._calculateKey()}\\"}"}`;
        this.sendSocket(msg);
    }

    setColor(color, intensity) {
        this.color = color;
        this.intensity = intensity;
        const value = intensity * Math.pow(2, 24) + color.r * Math.pow(2, 16) + color.g * Math.pow(2, 8) + color.b;
        const msg = `{"cmd": "write", "model": "gateway", "sid": "${this._sid}", "data": "{\\"rgb\\":${value}, \\"key\\": \\"${this._calculateKey()}\\"}"}`;
        this.sendSocket(msg);
    }

    // ---------- private methods ----------
    _calculateKey() {
        const cipher = crypto.createCipheriv('aes-128-cbc', this.parent.password, AQARA_IV);
        return cipher.update(this.token, 'ascii', 'hex');
    }
}