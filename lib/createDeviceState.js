...
module.exports = function createDeviceState(device) {
    return new Promise(function (resolve, reject) {
        for (attribute in device) {
            ...
            if (_value || _value === 0) {
                console.log ('before call : attribute = ' + attribute);
                gladys.deviceType.getByIdentifier({ deviceIdentifier: device.sid, deviceService: 'xiaomi', deviceTypeIdentifier: attribute })
                    .then((deviceType) => {
                        console.log('OK => deviceIdentifier: ' + device.sid + ' deviceTypeIdentifier: ' + attribute);
                        //console.log('attribute = ' + attribute + ', value = ' + device[attribute] + ', _value = ' + _value + ', id = ' + deviceType.id);
                    })
                    .catch((err) => {
                        console.log('KO => deviceIdentifier: ' + device.sid + ' deviceTypeIdentifier: ' + attribute);
