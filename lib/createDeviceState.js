...
module.exports = function createDeviceState(device) {
    return new Promise(function (resolve, reject) {
        for (let _attribute in device) {
            ...
            if (_value || _value === 0) {
                console.log ('before call : _attribute = ' + _attribute + ' _value = ' + _value);
                gladys.deviceType.getByIdentifier({ deviceIdentifier: device.sid, deviceService: 'xiaomi', deviceTypeIdentifier: _attribute })
                    .then((deviceType) => {
                        console.log('OK => deviceIdentifier: ' + device.sid + ' deviceTypeIdentifier: ' + _attribute + ' _value = ' + _value);
                        //console.log('_attribute = ' + _attribute + ', value = ' + device[_attribute] + ', _value = ' + _value + ', id = ' + deviceType.id);
                    })
                    .catch((err) => {
                        console.log('KO => deviceIdentifier: ' + device.sid + ' deviceTypeIdentifier: ' + _attribute + ' _value = ' + _value);
