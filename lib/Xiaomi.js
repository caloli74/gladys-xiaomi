...
module.exports = class Xiaomi extends events.EventEmitter {
    ...
    constructor(password) {
        ...
        // schedule.scheduleJob('0 0 * * * *', function () { _reportDevices(); });
        schedule.scheduleJob('0 * * * * *', function () { this._reportDevices.bind(this); });
    }
    ...
    _reportDevices() {
        for (let sid in this.devices) {
            ...
            if (answer && this.ready) {
                console.log(answer.msg + ' ' + JSON.stringify(answer.data))
                this.emit(answer.msg, answer.data);
            }
        }
    }
